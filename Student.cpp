...
class Student
{
	...
	string lastname;
	string fathername;
	// нарушение ¬џ—ќ ќ√ќ —÷≈ѕЋ≈Ќ»я:
	// студент берЄт на себ€ задачи, которые решать не должен!!!
	// вместо этих трЄх полей Ћ”„Ў≈...
	//int birth_day = 15; // 1-31
	//int birth_month = 10; // 1-12
	//int birth_year = 1998;
	Date* birthday;
	// Ё“» ѕќЋя убираем вообще, они студенту не нужны!!!
	//string eyes_color;
	//string hair_color;
	//double growth;
	//double weight;
	...
	// 1) пам€ть расходуетс€ сразу, при том что изначально оценок нет вообще
	// 2) добавить 101-ый элемент в массив нельз€
	// 3) даже если сделать динамический массив, то возникает вопрос: почему тип данных —тудент должен брать на себ€ ответственность за ¬џƒ≈Ћ≈Ќ»≈ ѕјћя“» под массив, ѕ≈–≈¬џƒ≈Ћ≈Ќ≈»≈, проверку диапазонов баллов от 1 до 12
...
{
	setlocale(LC_ALL,"ru");
	//srand(time(0)); // дл€ генерации случайных чисел
	//Date obj1 = Date();
